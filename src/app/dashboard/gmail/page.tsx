...
export default function GmailPage() {
  ...
  const [selectedDetail, setSelectedDetail] = useState<GmailMessage | null>(
    null,
  );
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const res = await apiFetch(
        `/api/gmail/messages?limit=30&cursor=${nextCursor}`,
      );
      if (!res.ok) {
        setError(`Server error: ${res.status}`);
        return;
                  ...
                  return (
                    ...
                    >
                      <div className="flex shrink-0 flex-col items-center gap-1.5 pt-0.5">
                        <div
                          className={`h-4 w-4 rounded-full border ${highlight ? "border-yellow-500 bg-yellow-500/10" : "border-zinc-600"}`}
                        />
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-baseline justify-between gap-2">
                          <span
                            className={`truncate text-sm ${unread ? "font-semibold text-zinc-100" : "text-zinc-300"}`}
                          >
                            {msg.from ?? "Unknown"}
                          </span>
                        ...
                        <div
                          className={`mt-0.5 truncate text-sm ${unread ? "font-medium text-zinc-200" : "text-zinc-400"}`}
                        >
